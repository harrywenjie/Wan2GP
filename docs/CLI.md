...
- `artifacts` – list of objects describing every saved file. Each entry captures: `role` (`foreground`, `alpha`, `audio`, `mask_archive`, etc.), `path` (absolute string), `container` (file container/extension), `codec` (video or audio codec when known), `frames` (integer when applicable), `duration_s` (float, optional), and `metadata_sidecar` (path to JSON/embedded indicator, `null` when embedded metadata was written). MatAnyOne audio artifacts additionally surface `sample_rate`, `language`, and `channels` so remux automation can align tracks without re-reading sidecars. Future writers may extend entries with additional keys; consumers must ignore unknown fields.
...
The manifest writer must flush the JSON line only after persistence succeeds. Dry runs skip manifest emission entirely. MatAnyOne reuses the same format; its `artifacts` list uses `mask_foreground`, `mask_alpha`, `rgba_archive`, and (when audio tracks are reattached) `audio` roles enriched with `sample_rate`, `duration_s`, `language`, and `channels`.
...
- Canonical audio tracks persisted via `MediaPersistenceContext.save_audio` (one file per decoded source stream). Files adopt the configured audio format (`server_config.audio_output_format`, defaulting to WAV) and receive JSON sidecars when running with `--metadata-mode json`. The CLI now logs each saved track with its path, sample rate, derived duration, and language tag; the same fields land in `result.metadata["audio_tracks"]` for downstream consumers.
- A manifest line appended to `<output_dir>/manifests/run_history.jsonl` (or `--manifest-path`), capturing inputs and artifacts with `mask_foreground`, `mask_alpha`, `rgba_archive`, and (when applicable) `audio` roles. MatAnyOne audio artifacts now include `sample_rate`, `duration_s`, `language`, and `channels` alongside container/codec metadata.

MatAnyOne now threads the per-run `MediaPersistenceContext` supplied by `ProductionManager` and records artifact manifests alongside CLI runs. Video writes honour container/codec overrides (with `--codec` acting as a per-run override), audio tracks are decoded/remuxed via `_persist_audio_artifacts`, persisted through `save_audio` (respecting template sample rates and formats), and reattached onto the foreground video while the standalone files surface in manifests. Mask archives respect the `save_masks` toggle so debug bundles only materialise when explicitly configured. Persistence helpers retain retry logging so automation can detect and react to IO errors deterministically.
Automated coverage: `tests/test_matanyone_cli_integration.py` patches the heavy pipeline while invoking the CLI end-to-end, asserting the manifest captures `mask_foreground`, `mask_alpha`, `rgba_archive`, and `audio` artifacts plus JSON sidecars when running with `--metadata-mode json`.
