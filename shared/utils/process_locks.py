...
_LOGGER = logging.getLogger("wan2gp.cli")


def _emit_wait_notice(process_name: str, notifier: Optional[Callable[[str], None]]) -> None:
    message = f"Process {process_name} is suspended while waiting for GPU resources"
    if notifier:
        notifier(message)
    else:
...
def acquire_GPU_ressources(
    ...
    if original_process_status is not None:
        ...
        while True:
            ...
            if round(total_wait, 2) >= 5 and not wait_msg_displayed:
                wait_msg_displayed = True
                _emit_wait_notice(process_name, notifier)
